/* AI: Movies - Create AskMovieQuestion Stored Procedure */

USE MoviesDB
GO

-- Create a stored procedure that can call Azure OpenAI to chat using a completion model
CREATE OR ALTER PROCEDURE AskMovieQuestion
...
BEGIN
	...
	DECLARE @Payload varchar(max) = JSON_OBJECT(
		'messages': JSON_ARRAY(
			JSON_OBJECT('role': 'system', 'content': 'You are an assistant that provides a synopsis of the movie title supplied by the user.'),
			JSON_OBJECT('role': 'user', 'content': CONCAT('The movie title is: "', @MovieTitle, '"'))
		),
