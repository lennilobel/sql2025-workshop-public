...
-- Need columnstore indexes for batch mode to demonstrate adaptive joins
CREATE NONCLUSTERED COLUMNSTORE INDEX IX_bigProduct_ProductID ON bigProduct(ProductID)
CREATE NONCLUSTERED COLUMNSTORE INDEX IX_bigTransactionHistory_ProductID ON bigTransactionHistory(ProductID)		-- ~ 1 min


/*

-- Drop clustered rowstore indexes created by Adam M (22 sec)
ALTER TABLE bigProduct DROP CONSTRAINT pk_bigProduct
ALTER TABLE bigTransactionHistory DROP CONSTRAINT pk_bigTransactionHistory

-- Add clustered columnstore indexes (48 sec)
CREATE CLUSTERED COLUMNSTORE INDEX IX_bigProduct_CCI ON bigProduct
CREATE CLUSTERED COLUMNSTORE INDEX IX_bigTransactionHistory_CCI ON bigTransactionHistory

-- I'm NOT getting adaptive joins... but I should because I've got clustered columnstore indexes!

-- Drop the clustered columnstore indexes (25 sec)
DROP INDEX IX_bigProduct_CCI ON bigProduct
DROP INDEX IX_bigTransactionHistory_CCI ON bigTransactionHistory

-- Add nonclustered columnstore indexes (11 sec)
CREATE NONCLUSTERED COLUMNSTORE INDEX IX_bigProduct_ProductID ON bigProduct(ProductID)
CREATE NONCLUSTERED COLUMNSTORE INDEX IX_bigTransactionHistory_ProductID ON bigTransactionHistory(ProductID)

-- Still not getting adaptive joins... but that's because I've got no clustered indexes at all.

-- Re-add Adam's clustered rowstore indexes (48 sec)
ALTER TABLE bigProduct ADD CONSTRAINT pk_bigProduct PRIMARY KEY (ProductId)
ALTER TABLE bigTransactionHistory ADD CONSTRAINT pk_bigTransactionHistory PRIMARY KEY (TransactionID)

-- Only now do I get adaptive joins... why?!?
